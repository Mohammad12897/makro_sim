...
**Ziel**  
Backtest für einen ETF ausführen und Ergebnisse (`portfolio_value`, `metrics`, `weights_over_time`) zuverlässig speichern und verifizieren.
...
- `price_close` DataFrame vorhanden  
- `weights_for_backtest` vorhanden  
...
Backtest ausführen
Was passiert?
Die Backtest‑Funktion simuliert die historische Entwicklung des Portfolios anhand von:
...
→ Grundlage für Reporting und Entscheidungen.
try:
...
Begriffe
Portfolio Value
Zeitreihe des Gesamtwerts des Portfolios (Index = Datum). Grundlage für Renditen und Drawdowns.
...
Weights Over Time
DataFrame mit Datum × Assets; zeigt:
...
KeyError in results → immer setdefault() verwenden

Datei wird nicht geschrieben → Schreibrechte prüfen

Syntaxfehler durch Fremdtext → Browser‑Fragmente entfernen
