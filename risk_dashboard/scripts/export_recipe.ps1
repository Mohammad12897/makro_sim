# risk_dashboard/scripts/export_recipe.ps1
param(
    [switch]$GitAdd = $false,
    [switch]$CopyToClipboard = $true
)

# Ensure script runs from project root (optional)
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Definition
Set-Location -Path (Resolve-Path "$scriptDir\..") | Out-Null

# Create docs directories
$docsDir = ".\risk_dashboard\docs"
$imagesDir = Join-Path $docsDir "images"
New-Item -ItemType Directory -Path $docsDir -Force | Out-Null
New-Item -ItemType Directory -Path $imagesDir -Force | Out-Null

# Markdown content (Here-String). Edit content below if needed.
$md = @'
# Backtest Rezept
...
$mdPath = Join-Path $docsDir "backtest-recipe.md"
$md | Out-File -FilePath $mdPath -Encoding UTF8 -Force

Write-Host "Wrote:" (Resolve-Path $mdPath)

if ($CopyToClipboard) {
    try {
        Get-Content $mdPath -Raw | Set-Clipboard
...
if ($GitAdd) {
    if (Get-Command git -ErrorAction SilentlyContinue) {
        git add $mdPath
        Write-Host "Staged file in git index."
    } else {
        Write-Warning "Git not found in PATH; cannot stage file."
    }
}
